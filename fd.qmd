...
Thus, the hypothesis is that in both GCS and JPS models the agent speed decreases as the environment becomes more populated.
...
Also, in this example I relied on classical density measure but there are other ways to measure it. For example, [Duives et al. (2015)](https://www.sciencedirect.com/science/article/pii/S037843711200667X) discuss 7 measures for quantifying pedestrian crowdedness in which Voronoi and X-T measures are considered "the best". It could be useful to explore different methods to confirm (or not) that the negative slope remains in GCS and the current JPS models.

Finally, it could be interesting to experiment with different agent numbers in JPS to explore when/how congestion occurs, thus leading to the expected outcome -- as density increases, speed/velocity decreases.
